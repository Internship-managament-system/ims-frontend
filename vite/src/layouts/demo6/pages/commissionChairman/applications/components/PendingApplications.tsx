...
const PendingApplications: React.FC<PendingApplicationsProps> = ({ limit }) => {
  ...
  const statusTabs = [
    ...
    { key: '⏳ İnceleme Bekliyor', label: 'İnceleme Bekliyor', count: applications.filter(app => app.status === '⏳ İnceleme Bekliyor').length },
    { key: '❌ Eksik Belgeler', label: 'Eksik Belgeler', count: applications.filter(app => app.status === '❌ Eksik Belgeler').length },
